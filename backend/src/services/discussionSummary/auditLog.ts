...
export interface AuditEventInput {
  ...
  /** The `Transcript.id` this discussion-summary run is for. Not unique across attempts —
...
/**
 * Single write path for the discussion-summary audit trail (STORY-009), mirroring
 * `meetingSummary/auditLog.ts` and `segmentMarking/auditLog.ts`. Every call gets its own fresh
 * `auditEventId` (UUID v4), so multiple entries about the same transcript stay individually
 * traceable even though they share `resourceId`. Satisfies the story's Trust criterion:
 * discussion point summaries and review flags are both logged through this path.
 */
