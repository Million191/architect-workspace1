...
export interface AuditEventInput {
  ...
  /** The `Transcript.id` this decision-listing run is for. Not unique across attempts — dedup
   * hits and repeated failures share it — so `auditEventId` is what makes each entry distinct. */
  resourceId: string;
...
/**
 * Single write path for the decision-extraction audit trail (STORY-010), mirroring
 * `discussionSummary/auditLog.ts` and `meetingSummary/auditLog.ts`. Every call gets its own
 * fresh `auditEventId` (UUID v4), so multiple entries about the same transcript stay
 * individually traceable even though they share `resourceId`. Satisfies the story's Trust
 * criterion: decision-listing attempts and results are both logged through this path.
 */
